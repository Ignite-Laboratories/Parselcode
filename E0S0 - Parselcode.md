...
_Sending_ something like a color or index 42 is pretty standard - but sending _how to **derive**_ 
a color _**isn't.**_
...
changing methods at runtime - the term, however, was coined by graphical design engineers as a shorthand for quickly 
...
Most importantly, however, you can _**chain**_ index accessors in the swizzling pattern!  That's inherently how
we achieve arbitrary swizzling of "vectorized components" =) 

    var data []any
    []any{third, first, secondA, secondB} = swizzle(data, [2], [0], [1], [1])
...
If the external variable shares a name with a member of the target, the _target's member_ is **always** selected.  In those
...
the `std.Cursorable` interface - which literally defines _"six degrees of semantic freedom,"_ broken into two
...
`Yield()` - or, "peek" the current element it's located at using `Current()` without affecting the chain.  The steps defined 
above, however, can be expressed in a much friendlier way - similar to index accessors, but in what I call the _"cursor 
accessor"_ pattern.  This pattern facilitates the LIQs described at the start of this enigma, but we'll circle back to
that later.  Cursor accessors provide _compile time_ instructions for **intelligently** querying data.
...
re-invent the way we _loop_ through a data set - instead, we get to define how to logically _traverse_ it!
    ...
    tl;dr - a cursor logically selects data -while- traversing through it

Think of it like a cursor in your IDE jumping around to 'select' blocks of code =)
...
If you'd like to chain operations together, you do so with a `;` character:
...
everything BUT the elements in this range" - a kind of "full outer join," in our SQL friends' terminology!
     ...
     <<42=99>  | Relative | Over/Under Flow | JumpTo(𝑖) - Same as above, but underflows and includes [end, 100], instead (NOTE: The single ending bracket!)
...
Just as with index accessors, you can also use _**variables**_ as your operands!  That means you can define 
the relative motion of how to _dynamically_ "select" a region of space - without reflection and entirely through compiled 
code.  Earlier I emphasized this is a form of _**intelligent**_ querying, and that's exactly what I meant - because,
as we will see in the next step, this includes the ability to _parse_ functions at runtime.

    fn := func() (i int) { 
        ... // Calculate an index
        return i
    }
    v := data[42:fn, 4] // Walk from element 42 to the -runtime- result of variable 'func()' referenced by 'fn' at a stride of 4

In traditional index access, you must provide an _expression._  To JanOS, we're aware you would like to replace the referenced
function at runtime - so we wire the request up to invoke the function, if possible.

But we haven't even gotten to the _**coolest**_ part, yet - _predicates!_  These are functions that get called with
each element _**during traversal**_ to either mutate or filter it out.  At any point in a chain, you can insert a trinity
...
'distinctly' (in SQL terminology), simply prefix those you wish to do so with a `!` character.
    ...
    // Cursor access with two distinct predicates
    [22];[42:99, 4];[!SelectFn, ForEachFn, !TransformFn]
...
Finally, a special keyword is used to convert abstract regions of code into a `liq.Lick` structure - `parse(...)` -
which provides you with two operations:

    l := parse(data<<42=99, 4>>;[!SelectFn]) // Creates a liq.Lick

    l.Yield() // Re-invoke the LIQ on-demand
    l.Produce() // Outputs the LIQ as runtime created anonymous serializable structure

It also operates off of any callable chain of code.

                 ⬐ The source of the chain becomes the required starting type of the lick
    l := parse(myMap["42"].Execute().SomeField)
                       ⬑ Each step to invoke is what can be serialized from a similar type

The `parse` keyword takes an _expression_ or _statement_ and yields a `liq.Lick` that can be produced
into a serializable format describing _how_ to re-execute those steps.  In addition to chained member
accessors, `parse` can accept _swizzle_ expressions
...
When a lick is created, its yield function directly invokes the originally given code.  So long
as you retain the original structure, you will have minimal performance losses.  Once "produced"
into a serializable structure, reflection may be relied upon to repeat the same steps.  Given that
a neural architecture aims to pass licks around like _candy_, you should make peace that reflection
is a crucial element of efficient and mature execution - with the very same applying to _**Life!**_

The `liq` package holds all the serializable types that represent different steps in the parsed query.
